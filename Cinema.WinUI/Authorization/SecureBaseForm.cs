...
namespace Cinema.WinUI.Authorization
{
    public partial class SecureBaseForm : Form
    {
        ...
        private IList<string> _formPrincipal;
        ...
        public SecureBaseForm(string[] roles, IList<string> userPrincipal)
        {
            if (!DesignMode)
            {
                this.IsMainWindow = false;
                ...
                this.Load += UserIsAllowed;
                this.Load += UserIsDenied;
        ...
        private void ValidateUserRoles()
        {
            foreach (string role in _formRoles)
                if (_formPrincipal.Contains(role))
        ...
        private void Show(CalledShowMethod calledShowMethod, IWin32Window owner)
        {
            if (!DesignMode)
            {
                if (UserCanOpenForm)
                {
                    ...
                    UserIsAllowed?.Invoke(this, new EventArgs());
                }
                else
                {
